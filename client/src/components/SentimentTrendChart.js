    ...
    const fetchSentimentData = async () => {
      try {
        ...
        news.forEach((item) => {
          ...
          if (!sentimentCount[date]) sentimentCount[date] = { positive: 0, negative: 0, neutral: 0 };
        ...
        setChartData({
          labels,
          datasets: [
            { label: "Positive", data: positiveData, borderColor: "green", backgroundColor: "rgba(0,255,0,0.2)" },
            { label: "Negative", data: negativeData, borderColor: "red", backgroundColor: "rgba(255,0,0,0.2)" },
            { label: "Neutral", data: neutralData, borderColor: "gray", backgroundColor: "rgba(128,128,128,0.2)" },
          ],
        });
      } catch (err) {
